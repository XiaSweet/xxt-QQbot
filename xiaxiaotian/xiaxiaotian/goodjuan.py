...
async def upgrade(session: CommandSession):
    ...
    if upgrade == 'ERROR-CR404':
    ...
    elif upgrade == 'ERROR-CRNotCallMe':
        ...
        await session.send(upgrade_report,at_sender=True)
        logger.info('[部落查询]结果无错误并已查询完毕并发送给用户,任务结束')
async def get_upgrade(tag: str) -> str:
    upgrade = subprocess.getoutput("python lib/clashroyale/donationcard/goodjuan.py -o '%s'"%(tag))
    return f'{upgrade}'
@upgrade.args_parser
...
@on_natural_language(keywords={'升职','长老'},only_to_me=True)
async def _(session: NLPSession):
    return IntentCommand(80.0, 'upgrade')